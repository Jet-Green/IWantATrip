<script setup>
import { computed } from 'vue'
import Header from "../components/Header.vue";
import { useHead } from '@unhead/vue'
import { useRoute } from "vue-router"
const route = useRoute();
let title = computed(() => {
  if (route.path == '/guides') {
    return 'Гиды'
  }
  if (route.path == '/poster') {
    return 'Афиши'
  }
  return 'Места'
})
const VITE_API_URL = import.meta.env.VITE_API_URL
useHead({
  title: `${title.value}: города и веси`,
  meta: [
    {
      name: 'description',
      content: 'Парк, гостиница, кафе, музей, спорт, отдых, ресторан - места для туристов',
    },
    {
      property: 'og:title',
      content: 'Места: города и веси',
    },
    {
      name: 'og:description',
      content: 'Парк, гостиница, кафе, музей, спорт, отдых, ресторан - места для туристов',
    },
    {
      name: 'og:url',
      content: `${VITE_API_URL}/places`,
    },
  ],
  link: [
    { rel: 'canonical', href: `${VITE_API_URL}/places` }
  ]
})

</script>
<template>

  <a-layout>
    <Header></Header>
    <!-- в документации с margin'ом, чтобы предотвратить перекрывание контента хедром -->
    <a-layout-content style="min-height: 100vh; background-color: #FFFEF4; padding-top: 80px; padding-bottom: 80px;">
      <Suspense>
        <router-view v-slot="{ Component }">
          <transition name="fade" mode="out-in">
            <component :is="Component" />
          </transition>
        </router-view>
      </Suspense>
    </a-layout-content>
  </a-layout>
</template>
<style></style>
