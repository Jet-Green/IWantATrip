...
const previewImageUrl = ref(null); // For displaying the selected/cropped image preview
const currentImgBlob = ref(null); // To store the actual Blob data for upload
...
<template>
    ...
    <a-row :gutter="[16, 16]">
        <a-col v-for="g in guides" :key="g._id" :xs="24" :sm="12" :lg="8" :xl="6" class="mb-8">
                <a-card hoverable style="border-radius: 10px; height: 100%;">
                    <a-card-meta>
                        <template #title>
                             <div class="d-flex align-center">
                                <a-avatar :size="40" :src="g.image" class="mr-8"/>
    ...
    <a-modal v-model:open="addGuideModal" :footer="null" title="Добавить гида" @cancel="clearNewImageSelection">
            ...
            <a-row :gutter="[16, 16]">
                ...
                <!-- Image Upload Section -->
                 <a-col :xs="24">
                    <a-form-item label="Фотография">
                        <div class="image-preview-container mb-8" v-if="previewImageUrl">
                             <img :src="previewImageUrl" alt="Предпросмотр фото" class="preview-image" @error="handleImgError"/>
                        </div>
                        <a-button type="dashed" block @click="openImageCropper" class="ma-8">
                         ...
                         <a-button v-if="previewImageUrl" size="small" danger @click="clearNewImageSelection" class="mt-4">
                 ...
                 <a-col :span="24">
                     <a-form-item label="Локации / Города работы">
                        <a-input v-model:value="guide.location" placeholder="Казань, Болгар, Свияжск"/>
                    </a-form-item>
                </a-col>
                 <a-col :span="24">
                     <a-form-item label="Подробное описание" required>
                        <a-textarea v-model:value="guide.description" placeholder="Расскажите о себе, опыте, маршрутах..." :auto-size="{ minRows: 3, maxRows: 6 }"/>
                     </a-form-item>
                </a-col>
...
.ma-8 { margin: 8px; } /* Added margin for button spacing */
.mr-1 { margin-right: 4px; } /* Added margin for icon spacing */
.text-right { text-align: right; } /* Added for modal buttons */
