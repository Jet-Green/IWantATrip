...
async function addContract() {
  let res = await contractStore.createContract(contractForm, contractForm.userEmails[0]);
...
onMounted(() => {
  if (Object.keys(contractStore.editingContract).length != 0) {
    contractForm = contractStore.editingContract
  }
})
onUnmounted(() => {
  contractStore.clearEditingContract()
})
</script>
<template>
  ...
  <form @submit.prevent="submit" class="ma-16">
    <a-row :gutter="[16, 16]">
      <a-col :span="24">
        Полное наименование
        <a-input placeholder="Общество с ограниченной ответственностью 'Огонёк'" v-model:value="contractForm.fullName"
          required></a-input>
      </a-col>
      <a-col :span="24" :md="12">
      ...
      <a-col :xs="12">
        Описание платежа
        <a-input placeholder='Платформа \"Города и Веси\" оплата тура' v-model:value="contractForm.bankAccount.details"
          disabled></a-input>
      </a-col>
      <a-col :span="24">
        Комиссия
