...
async function deleteTrack(_id) {
  ...
  await refreshTracksOnModeration()
}

async function refreshTracksOnModeration() {
  let data = await trackStore.getAll(page, query)
  tracksOnModeration.value = data
}
...
<template>
  <a-row :gutter="[8, 8]" class="mt-8">
    <a-col
      ...
      :sm="12"
      :xs="24"
      v-if="tracksOnModeration.length > 0"
      v-for="(track, index) of tracksOnModeration"
      :key="index"
    >
    ...
    <a-col :span="24" v-else> Нет маршрутов на модерации </a-col>
  </a-row>
</template>
