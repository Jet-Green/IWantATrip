...
let checkManagers = async () => {
  ...
  for (const manager of form.managers) {
    let result = await userStore.checkUserEmail(manager);
    emailExists.value.push(result.data);
  }
  isChecking = false;
}
        ...
        <Form :validation-schema="formSchema" v-slot="{ meta }" @submit="submit">
          <a-row :gutter="[16, 16]">
            ...
            <a-col :span="24">
              ...
              <div v-for="   item, index in form.managers   " :key="index" style="display: flex" align="center"
                class="mb-16">
                <a-input v-model:value="form.managers[index]" placeholder="manager@mail.ru" class="mr-8" />
